      ...
      <Row gutter={[12, 12]}>
        ...
        <Col xs={12} md={6} lg={5}>
          <Card size="small">
            ...
            />
            <Text type="secondary" style={{ fontSize: 11 }}>hit {cacheHitRate.toFixed(1)}%</Text>
          </Card>
        </Col>
      </Row>
      ...
      <Card size="small">
        <Table<SessionTokens>
          ...
          columns={[
            ...
            { title: "Input", dataIndex: "input_tokens", align: "right" as const, width: 100, render: (v: number) => formatTokens(v) },
            { title: "Output", dataIndex: "output_tokens", align: "right" as const, width: 100, render: (v: number) => formatTokens(v) },
            {
              title: "Cache Create",
            ...
            },
            {
              ...
              align: "right" as const,
              width: 110,
              render: (v: number) => <Text style={{ color: "#52c41a" }}>{formatTokens(v || 0)}</Text>
            },
            {
              title: "Total",
          ...
          summary={() => (
            <Table.Summary fixed>
              <Table.Summary.Row>
                ...
                <Table.Summary.Cell index={4} align="right"><Text strong>{formatTokens(totals.output)}</Text></Table.Summary.Cell>
                <Table.Summary.Cell index={5} align="right"><Text strong>{formatTokens(totals.cacheCreate)}</Text></Table.Summary.Cell>
                <Table.Summary.Cell index={6} align="right"><Text strong style={{ color: "#52c41a" }}>{formatTokens(totals.cacheRead)}</Text></Table.Summary.Cell>
                <Table.Summary.Cell index={7} align="right"><Text strong>{formatTokens(totals.total)}</Text></Table.Summary.Cell>
