...
function EmptyProjectHome() {
  return (
    <Result
      status="info"
      title="项目已创建，暂无业务页面"
      subTitle="当前项目已保留登录、布局、角色权限和请求基础设施。请根据业务需求添加路由页面。"
      extra={
        <Button type="primary">
...
export function FirstAccessibleRedirect() {
  const { status, isAuthenticated, error, user, retryCurrentUser } = useAuth();
  ...
  const firstPath = findFirstAccessibleRoute(appRoutes, user);
  if (firstPath) {
